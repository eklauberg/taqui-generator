...
export class SqliteLinkRepository implements ILinkRepository {
    ...
    async save(link: Link): Promise<void> {
        this.db.run(
            "INSERT OR REPLACE INTO links (key, url, contexto) VALUES (?, ?, ?)", [link.key, link.url, link.contexto]
        );
    }

    async load(key: string): Promise<Link> {
        const row = this.db.query<Link, [string]>("SELECT * FROM links WHERE key = ?").get(key);
